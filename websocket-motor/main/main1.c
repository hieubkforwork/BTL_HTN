...
#include "wifi.h"   // <- QUAN TRỌNG: dùng thư viện WiFi mới

static const char *TAG = "MOTOR_WS";

/* ======================= BIT FLAGS ======================= */
#define WIFI_CONNECTED_BIT BIT0
#define FIREBASE_POST_BIT  BIT1
...
/* ===========================================================
 *  CALLBACK: NHẬN IP TỪ wifi.c (AP → STA)
 * =========================================================== */
void wifi_on_sta_got_ip(esp_ip4_addr_t ip)
{
    sprintf(ip_str, IPSTR, IP2STR(&ip));
    ESP_LOGI(TAG, "WiFi READY. STA IP = %s", ip_str);
...
/* ===========================================================
 *  MAIN — DÙNG WIFI TỪ WEB CONFIG
 * =========================================================== */
void app_main(void)
{
    // BẮT BUỘC: init NVS để WiFi không báo lỗi
    ...
    ESP_LOGI(TAG, "Starting DRV8833 + WebSocket + Firebase (WiFi dynamic)...");

    wifi_event_group = xEventGroupCreate();

    // --- 1. BẮT ĐẦU WIFI Ở AP+STA TỪ THƯ VIỆN ---
    wifi_init_sta_or_ap();
    ESP_LOGI(TAG, "WiFi Config Portal active → connect to AP & enter SSID/PASS");

    // --- 2. CHỜ TỚI KHI NHẬN IP TỪ STA MODE ---
    EventBits_t bits = xEventGroupWaitBits(
        wifi_event_group,
        WIFI_CONNECTED_BIT,
        pdFALSE,
    ...
    // --- 5. TASK ADC + FIREBASE ---
    xTaskCreate(adc_task, "adc_task", 10000, NULL, 5, NULL);
    xTaskCreate(firebase_task, "firebase_task", 12000, NULL, 5, NULL);
}
